    ...
    try:
        ...
        def get_ls(y):
            ...
            col = ee.ImageCollection("LANDSAT/LC08/C02/T1_L2").merge(ee.ImageCollection("LANDSAT/LC09/C02/T1_L2")) \
                .filterBounds(geometry).filterDate(start, start.advance(4, 'month')).filter(ee.Filter.lt('CLOUD_COVER', 30))
            def mask(img):
        ...
        avg_lst_f = ee.ImageCollection([ee.Image(get_ls(y)) for y in [2022, 2023, 2024]]).mean().rename('AVG_LST_F')
        pred_2026_f = avg_lst_f.add(sen_slope_f.resample('bilinear').multiply(4)).clip(geometry)

        # STATS
        mean_v = avg_lst_f.reduceRegion(ee.Reducer.mean(), geometry, 30).getInfo().get('AVG_LST_F', 0)
        slope_v = sen_slope_f.reduceRegion(ee.Reducer.mean(), geometry, 1000).getInfo().get('slope', 0)
        ...
        st.error(f"Engine Error: {e}"); return None, None, None, None, None
