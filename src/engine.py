...
def get_gee_data(city_name: str, lon: float, lat: float):
    ...
    try:
        ...
        # 3. Current High-Res (Landsat 8/9) - WITH EMPTY CHECK
        landsat_col = ee.ImageCollection("LANDSAT/LC08/C02/T1_L2") \
            .merge(ee.ImageCollection("LANDSAT/LC09/C02/T1_L2")) \
            .filterBounds(aoi) \
            .filterDate("2024-01-01", "2027-01-01") \
            .filter(ee.Filter.lt("CLOUD_COVER", 30)) # Loosened for better results

        # If no images, stop here to prevent 'Image.multiply' error
        if landsat_col.size().getInfo() == 0:
            st.warning(f"No clear satellite images found for {city_name} (2024-2026).")
