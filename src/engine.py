...
def get_gee_data(city_name: str, lon: float, lat: float):
    ...
    try:
        ...
        # --- 1. MODIS 22-YEAR TREND (Regional Signal) ---
        ...
        # --- 2. LANDSAT 30m BASELINE (Local Detail) ---
        ...
        # Band Guard: If Landsat is empty, fallback to MODIS
        ...
        # 2026 Forecast = Baseline + (Slope * 2 years from 2024 median)
        pred_2026_f = avg_lst_f.add(slope_resampled.multiply(2)).clip(geometry)

        # --- 4. TILE URL GENERATION (The Bulletproof Fix) ---
        vis = {"min": 80, "max": 120, "palette": ['#0000ff', '#ffff00', '#ff0000']}
        
        # We generate signed map IDs from Earth Engine
        map_id_current = ee.data.getMapId({'image': avg_lst_f, 'visParams': vis})
        map_id_forecast = ee.data.getMapId({'image': pred_2026_f, 'visParams': vis})
        ...
        stats = {
            ...
            "vis_max": vis["max"],
            "palette": vis["palette"],
            "current_tile_url": map_id_current['tile_fetcher'].url_format,
            "forecast_tile_url": map_id_forecast['tile_fetcher'].url_format
        }
